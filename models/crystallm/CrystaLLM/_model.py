...
class GPT(nn.Module):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        # Track previous tokens and completion flags for each sequence in batch
        prev_ids = torch.full((batch_size,), -1, dtype=torch.long, device=idx.device)
        completed = torch.zeros(batch_size, dtype=torch.bool, device=idx.device)
        
        # Store results for each sequence
        results = [idx[i].clone() for i in range(batch_size)]
        active_indices = list(range(batch_size))
        
        while active_indices and len(results[active_indices[0]]) < len(idx[0]) + max_new_tokens:
            # Find max length among active sequences
            max_len = max(len(results[i]) for i in active_indices)
            
            # Pad and stack active sequences
            padded_sequences = []
            for i in active_indices:
                seq = results[i]
                if len(seq) < max_len:
                    padding = torch.zeros(max_len - len(seq), dtype=idx.dtype, device=idx.device)
                    seq = torch.cat([seq, padding])
                padded_sequences.append(seq)
            active_idx = torch.stack(padded_sequences)
            
            # if the sequence context is growing too long we must crop it at block_size
            idx_cond = active_idx if active_idx.size(1) <= self.config.block_size else active_idx[:, -self.config.block_size:]
            ...
            for i, orig_idx in enumerate(active_indices):
                results[orig_idx] = torch.cat((results[orig_idx], idx_next[i]), dim=0)
                
                # Check for double newlines
                if idx_next[i] == newline_id and prev_ids[orig_idx] == newline_id:
                    active_indices.remove(orig_idx)
                prev_ids[orig_idx] = idx_next[i]
        
        # Pad all sequences to the same length for final output
        max_len = max(len(seq) for seq in results)
        padded_results = torch.zeros((batch_size, max_len), dtype=idx.dtype, device=idx.device)
