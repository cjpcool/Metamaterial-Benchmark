...
class GraphAttentionTransformer(torch.nn.Module):
    def __init__(self,
        ...
        drop_path_rate=0.0,
        mean=None, std=None, scale=None, atomref=None, output_size=None):
        super().__init__()
        ...
        output_size = '1x0e' if output_size is None else output_size
        ...
        self.head = torch.nn.Sequential(
            ...
            LinearRS(self.irreps_feature, o3.Irreps(output_size), rescale=_RESCALE))
    ...
    def forward(self, f_in, pos, batch, node_atom, edge_d_index, **kwargs) -> torch.Tensor:
        edge_src, edge_dst = edge_d_index[0], edge_d_index[1]
...
def graph_attention_transformer_nonlinear_l2(irreps_in, radius, num_basis=128, 
    atomref=None, task_mean=None, task_std=None, output_size=None, **kwargs):
    model = GraphAttentionTransformer(
        irreps_in=irreps_in,
        irreps_node_embedding='128x0e+64x1e+32x2e', num_layers=6,
        irreps_node_attr='1x0e', irreps_sh='1x0e+1x1e+1x2e',
        ...
        norm_layer='layer',
        alpha_drop=0.2, proj_drop=0.0, out_drop=0.0, drop_path_rate=0.0,
        mean=task_mean, std=task_std, scale=None, atomref=atomref, output_size=output_size)
    return model
