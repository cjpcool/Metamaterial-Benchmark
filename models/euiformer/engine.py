...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for step, data in enumerate(data_loader):
        ...
        with amp_autocast():
            pred = model(f_in=data.node_feat, pos=data.cart_coords, batch=data.batch,
                node_atom=data.node_type if hasattr(data, 'node_type') else torch.argmax(data.node_feat,dim=1)+1,
            ...
            loss = criterion(pred.view(len(data.y), -1), (data.y[:, target] - task_mean) / task_std)
        ...
        end_time = time.time()
        mean_time_per_batch += (end_time - begin_time)
        # logging
    ...
    return mae_metric.avg, mean_time_per_batch/ len(data_loader)
...
def evaluate(model, norm_factor, target, data_loader, device, amp_autocast=None, 
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            targe_all.append(data.y[:, target])
    ...
    return mae_metric.avg, loss_metric.avg, r2, nrmse, mae
