...
import numpy as np
from models.geoldm.egnn.models import EGNN_dynamics_QM9, EGNN_encoder_QM9, EGNN_decoder_QM9

from models.geoldm.equivariant_diffusion.en_diffusion import EnVariationalDiffusion, EnHierarchicalVAE, EnLatentDiffusion

import pickle
from os.path import join

def get_model(args, device, dataset_info, dataloader_train):
    if len(dataset_info['n_nodes']) <=0:
        n_atoms = dataloader_train.dataset.data['num_atoms']
        histogram = {}
        for node_num in range(dataset_info['max_n_nodes']):
            histogram[node_num] = (node_num == n_atoms).sum()
    else:
...
def get_latent_diffusion(args, device, dataset_info, dataloader_train):
    ...
    if hasattr(args, 'ae_path') and args.ae_path is not None:
    ...
    if args.probabilistic_model == 'diffusion':
        vdm = EnLatentDiffusion(
            ...
            timesteps=args.diffusion_steps,
            noise_schedule=args.diffusion_noise_schedule,
            noise_precision=float(args.diffusion_noise_precision),
            loss_type=args.diffusion_loss_type,
            norm_values=args.normalize_factors,
            include_charges=args.include_charges
            )
...
class DistributionProperty:
    ...
    def _create_prob_given_nodes(self, values):
        ...
        (prop_min, _), (prop_max, _) = torch.min(values, dim=0), torch.max(values,dim=0)
        prop_range = prop_max - prop_min + 1e-12
        histogram = torch.zeros(n_bins, values.shape[1])
        for val in values:
            i = ((val - prop_min)/prop_range * n_bins).to(torch.int32)
            ...
            i[i==n_bins]-=1
                # i = n_bins - 1
            histogram[i] += 1
        probs = histogram / torch.sum(histogram, dim=0, keepdim=True)
        probs = Categorical(probs.t())
    ...
    def _idx2value(self, idx, params, n_bins):
        ...
        left = idx.to(torch.float32) / n_bins * prop_range + params[0]
        right = (idx+1).to(torch.float32) / n_bins * prop_range + params[0]
