...
from datasets_scripts.dataset_truss import LatticeStiffness, LatticeModulus
...
from datasets_scripts.baseDataset import LatticeTruss
from evaluation.eval_utils import lattice_params_to_matrix, frac_to_cart_coords
...
class LatticeEvaluator(LatticeEvaluatorMaster):
    def __init__(self,
                 test_dataset: LatticeTruss=None,
        ...
        if data_size_for_eval is not None:
            ...
            self.data_size_for_eval = len(test_dataset)
        assert self.cluster_size <= self.data_size_for_eval, 'data_size_for_eval < cluster_size'
        for lattice in self.test_dataset:
            frac_coord = lattice['frac_coords']      
            lattice_lengths = lattice['lengths'][0]
            lattice_angles = lattice['angles'][0]
            try:
                lattice_vector = lattice['vector']
            except:
            ...
            lattice['cart_coords'] = cart_coord 
    ...
    def eval_condition_effectiveness(self):
        ...
        print("size of selected data for eval: ", right)
        selected_idx = shuffle(idx_node_num)[:right]
        selected_data_for_eval = [self.test_dataset[i] for i in selected_idx]
        # print(selected_data_for_eval[0])
        train_x = [data['cart_coords'] for data in selected_data_for_eval]
        train_y =[data['mech_props'][0] for data in selected_data_for_eval]
    ...
    def eval_diversity(self):
        ...
        metrics_dict, _ = self.compute_cov(cart_coords, cart_coords, self.diversity_error_bar)
        print(metrics_dict)
    ...
    def __read_eval_data(self, eval_file_path):
        ...
        full_path = eval_file_path
        lattices = pickle.load(open(full_path, 'rb'))
        for lattice in lattices:
            frac_coord = lattice['coordinates']
            self.frac_coords.append(frac_coord)
            ...
            self.cart_coords.append(cart_coord)
            if 'mech_props' in lattice:
                self.cond_prop.append(lattice['mech_props'])
    ...
    def graph_validity(self, coords: List, edges: List, lattice_vectors: List):
        ...
        periodicity_ratio = np.array(periodicity).sum() / self.gen_size
        print(f"Periodicity rate: {periodicity_ratio}")
        mean_symmetry = np.array(symmetry_ratio).sum() / self.gen_size
        print(f"Mean Central Symmetry rate: {mean_symmetry}")
        connectivity_ratio = np.array(connectivity).sum() / self.gen_size
        print(f"Connectivity rate: {connectivity_ratio}")
        dangling_node_ratio = 1- np.array(dangling_node).sum() / self.gen_size
    ...
    def has_dangling_node(coords, edge_index):
        if edge_index is None or (isinstance(edge_index, np.ndarray) and (edge_index == np.array(None)).any()) or len(edge_index) == 0:
            return True
        ...
        i = np.clip(i, 0, len(coords)-1)
        j = np.clip(j, 0, len(coords)-1)
    ...
    def is_connected(edges):
        if edges is None or (edges == np.array(None)).any() or edges.shape[0] == 0:
            return False
        if edges.shape[0] == 2:
            edges = edges.T
        G = nx.Graph()
...
if __name__ == '__main__':
    ...
    evaluator = LatticeEvaluator(test_datset=test_data[:1000], eval_file_path='D:\\Workspace\\PhD_workspace\\metabench-proj\\Metamaterial-Benchmark\\gen_resutls\\cond-cdvae\\save_results')
